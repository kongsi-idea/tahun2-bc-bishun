/* ================================================================
   ...
   架构照 tahun1-bc-bishun，换数据 + 换皮（任天堂 / 马力欧）。
   ================================================================ */
(function () {
  "use strict";
  ...
  function progKey() {
    ...
    return "bishun2_prog__" + pc + "__" + nm;
  }
  function load() {
  ...
  /* ---------- 声音（WebAudio，生成式，可静音） ---------- */
  var Sfx = (function () {
    var ctx = null, muted = localStorage.getItem("bishun2_muted") === "1";
    function ac() {
    ...
    return {
      ...
      toggle: function () { muted = !muted; localStorage.setItem("bishun2_muted", muted ? "1" : "0"); if (!muted) this.resume(); return muted; },
  ...
  async function fetchRemoteProgress() {
    ...
    try {
      var res = await withTimeout(supabaseClient
        .from("tahun2_bc_bishun_progress")
        .select("chars_json")
  ...
  async function pushProgress() {
    if (!HAS_DB || !student || !student.playCode) return;
    try {
      await withTimeout(supabaseClient.rpc("submit_tahun2_bc_bishun_progress", {
  ...
  function countDoneFor(pc, nm) {
    try {
      var p = JSON.parse(localStorage.getItem("bishun2_prog__" + (pc || "guest") + "__" + nm)) || {};
      return Object.keys(p).filter(function (k) { return p[k] === 2; }).length;
    } catch (e) { return 0; }
  }

  var CODE_KEY = "bishun2_code";
  ...
  var COLORS = {
    stroke: "#3A2417", outline: "#C9B48F", drawing: "#2E86DE", highlight: "#2FAE58"
  };
  ...
  function startQuizFlow(ch, hintAfter, onDone) {
    writer.quiz({
      ...
      onMistake: function () { Sfx.miss(); flash("#E8452E"); },
